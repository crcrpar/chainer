...
TEST_P(MathTest, Relu) {
    Array a = testing::BuildArray({3, 1}).WithData<float>({-1.f, 2.f, 0.f});
    Array e = testing::BuildArray({3, 1}).WithData<float>({0.f, 2.f, 0.f});

    Run([&]() { testing::CheckForward([](const std::vector<Array>& xs) { return std::vector<Array>{Relu(xs[0])}; }, {a}, {e}); });
}

TEST_P(MathTest, ReluBackward) {
    using T = double;
    Shape shape{2, 3};
    Array a = (*testing::BuildArray(shape).WithLinearData<T>(1).WithPadding(1)).RequireGrad();
    Array go = testing::BuildArray(shape).WithLinearData<T>(-0.1, 0.1).WithPadding(1);
    ...
    CheckBackward([](const std::vector<Array>& xs) -> std::vector<Array> { return {Relu(xs[0])}; }, {a}, {go}, {eps});
}

TEST_P(MathTest, ReluDoubleBackward) {
    using T = double;
    Shape shape{2, 3};
    Array a = (*testing::BuildArray(shape).WithLinearData<T>(1).WithPadding(1)).RequireGrad();
    Array go = (*testing::BuildArray(shape).WithLinearData<T>(-0.1, 0.1).WithPadding(1)).RequireGrad();
    ...
    CheckDoubleBackwardComputation(
            [](const std::vector<Array>& xs) -> std::vector<Array> { return {Relu(xs[0])}; }, {a}, {go}, {ggi}, {eps, eps});
}

...
def _group_normalization(expander, gamma, beta, x, mean, var, eps, test):
    mean = mean[expander]
    if test:
...
@testing.parameterize(*(testing.product({
    ...
    'track_statistics': [True, False],
    })))
class GroupNormalizationTest(unittest.TestCase):

    def setUp(self):
        self.expander = (None, Ellipsis), (None,) * self.ndim
        self.aggr_axes = (0,) + tuple(range(2, self.ndim + 2))

        self.link = links.GroupNormalization(
            self.ngroups, self.nchannels, dtype=self.dtype)
        gamma = self.link.gamma.array
        gamma[...] = numpy.random.uniform(.5, 1., gamma.shape)
        beta = self.link.beta.array
        beta[...] = numpy.random.uniform(.5, 1., beta.shape)
        self.link.cleargrads()

        self.gamma = gamma.copy()[self.expander]  # fixed on CPU
        self.beta = beta.copy()[self.expander]  # fixed on CPU

        shape = (5, self.nchannels, 2,) * self.ndim
        self.x = numpy.random.uniform(-1, 1, shape).astype(self.dtype)
        self.reshapeed_x = numpy.reshape(self.x, (1, 5 * self.ngroups, -1))
        self.gy = numpy.random.uniform(-1, 1, shape).astype(self.dtype)

        if self.test:
            self.mean = numpy.random.uniform(
                -1, 1, (5 * self.ngroups,)).astype(self.dtype)
            self.var = numpy.random.uniform(
                0.5, 1, (5 * self.ngroups,)).astype(self.dtype)
...
class TestPopulationStatistics(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ngroups = 3
        self.nchannels = 6
        self.link = links.GroupNormalization(self.ngroups, self.nchannels, self.decay, self.eps)
        self.x = numpy.random.uniform(
            -1, 1, (self.nx, self.nchannels)).astype(numpy.float32)
        self.y = numpy.random.uniform(
            -1, 1, (self.ny, self.nchannels)).astype(numpy.float32)
...
class GroupNormalizationTestWithoutGammaAndBeta(unittest.TestCase):

    def setUp(self):
        self.ngroups = 3
        self.nchannels = 6
        self.link = links.GroupNormalization(
            self.ngroups, self.nchannels, use_gamma=False, use_beta=False)
        if self.test:
...
class TestDefaultInitializer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ngroups = 3
        self.nchannels = 6
        self.link = links.GroupNormalization(self.nchannels, self.decay)

    def test_initialize_cpu(self):
        testing.assert_allclose(numpy.ones(self.ngroups), self.link.gamma.data)
        testing.assert_allclose(numpy.zeros(self.ngroups), self.link.beta.data)
...
class TestInvalidInput(unittest.TestCase):
    ...
    def setUp(self):
        self.link = links.GroupNormalization(1, 3)

...
class TestInstanceNormalization(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if self.test and self.track_avg_stats:
            mean = numpy.random.uniform(-1, 1, self.shape[1]).astype(dtype)
            self.link.avg_mean[...] = mean
            var = numpy.random.uniform(0.5, 1, self.shape[1]).astype(dtype)
        ...
        self.check_backward_options = {
            'dtype': self.dtype, 'atol': 1e-4, 'rtol': 1e-3}
        if self.dtype == numpy.float16:
            self.check_forward_options.update({'atol': 1e-3, 'rtol': 1e-2})
            self.check_backward_options.update({'atol': 5e-1, 'rtol': 1e-1})
    ...
    @attr.cudnn
    @attr.gpu
    def test_forward_gpu_with_cudnn(self):
        self.link.to_gpu()
    ...
    def check_backward(self, x_data, y_grad, use_cudnn='never'):
    ...
    @attr.cudnn
    @attr.gpu
    def test_backward_gpu_with_cudnn(self):
        self.link.to_gpu()
        self.check_backward(
            cuda.to_gpu(self.x), cuda.to_gpu(self.gy), 'always')
...
class TestDefaultInitializer(unittest.TestCase):
    ...
    def test_initialize_cpu(self):
        ...
        testing.assert_allclose(
            numpy.zeros(self.size), self.link.beta.array)
